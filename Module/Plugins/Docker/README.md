...
<!-- Generated by Tools/Update-Readme.ps1 from the comment-based help in Docker.psm1. -->
<!-- Edit the help in the source, then re-run the tool. Changes here are overwritten. -->

| Command | Alias | Description |
| --- | --- | --- |
| `Invoke-DockerBuild` | `dbl` | Builds a Docker image from a Dockerfile. |
| `Invoke-DockerContainerExec` | `dxc` | Runs a command in a running container. |
| `Invoke-DockerContainerExecInteractive` | `dxcit` | Runs a command in a running container interactively. |
| `Invoke-DockerContainerInspect` | `dcin` | Displays detailed information on one or more containers. |
| `Invoke-DockerContainerList` | `dcls` | Lists running containers. |
| `Invoke-DockerContainerListAll` | `dclsa` | Lists all containers (running and stopped). |
| `Invoke-DockerContainerLogs` | `dlo` | Fetches the logs of a container. |
| `Invoke-DockerContainerPort` | `dpo` | Lists port mappings for a container. |
| `Invoke-DockerContainerRemove` | `drm` | Removes one or more containers. |
| `Invoke-DockerContainerRemoveForce` | `drm!` | Force removes one or more containers. |
| `Invoke-DockerContainerRestart` | `drs` | Restarts one or more containers. |
| `Invoke-DockerContainerRun` | `dr` | Runs a command in a new container. |
| `Invoke-DockerContainerRunInteractive` | `drit` | Runs a command in a new interactive container. |
| `Invoke-DockerContainerStart` | `dst` | Starts one or more stopped containers. |
| `Invoke-DockerContainerStop` | `dstp` | Stops one or more running containers. |
| `Invoke-DockerImageBuild` | `dib` | Builds a Docker image. |
| `Invoke-DockerImageInspect` | `dii` | Displays detailed information on one or more images. |
| `Invoke-DockerImageList` | `dils` | Lists Docker images. |
| `Invoke-DockerImagePrune` | `dipru` | Removes all unused images. |
| `Invoke-DockerImagePush` | `dipu` | Pushes an image to a Docker registry. |
| `Invoke-DockerImageRemove` | `dirm` | Removes one or more images. |
| `Invoke-DockerImageTag` | `dit` | Creates a tag for an image. |
| `Invoke-DockerNetworkConnect` | `dncn` | Connects a container to a network. |
| `Invoke-DockerNetworkCreate` | `dnc` | Creates a Docker network. |
| `Invoke-DockerNetworkDisconnect` | `dndcn` | Disconnects a container from a network. |
| `Invoke-DockerNetworkInspect` | `dni` | Displays detailed information on one or more networks. |
| `Invoke-DockerNetworkList` | `dnls` | Lists Docker networks. |
| `Invoke-DockerNetworkRemove` | `dnrm` | Removes one or more networks. |
| `Invoke-DockerPs` | `dps` | Lists running containers. |
| `Invoke-DockerPsAll` | `dpsa` | Lists all containers (running and stopped). |
| `Invoke-DockerPull` | `dpu` | Pulls an image from a Docker registry. |
| `Invoke-DockerStats` | `dsts` | Displays a live stream of container resource usage statistics. |
| `Invoke-DockerStopAll` | `dsta` | Stops all running containers. |
| `Invoke-DockerTop` | `dtop` | Displays the running processes of a container. |
| `Invoke-DockerVolumeInspect` | `dvi` | Displays detailed information on one or more volumes. |
| `Invoke-DockerVolumeList` | `dvls` | Lists Docker volumes. |
| `Invoke-DockerVolumePrune` | `dvprune` | Removes all unused local volumes. |
