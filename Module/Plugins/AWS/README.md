...
<!-- Generated by Tools/Update-Readme.ps1 from the comment-based help in AWS.psm1. -->
<!-- Edit the help in the source, then re-run the tool. Changes here are overwritten. -->

| Command | Alias | Description |
| --- | --- | --- |
| `Clear-AWSState` |  | A PowerShell function that clears the AWS state file. |
| `Get-AWSCurrentProfile` | `agp` | A PowerShell function that displays the currently active AWS profile. |
| `Get-AWSCurrentRegion` | `agr` | A PowerShell function that displays the currently active AWS region. |
| `Get-AWSProfiles` | `aws-profiles` | A PowerShell function that lists all available AWS profiles. |
| `Get-AWSPromptInfo` |  | A PowerShell function that gets AWS information for prompt display. |
| `Get-AWSRegions` | `aws-regions` | A PowerShell function that lists all available AWS regions. |
| `Initialize-AWSState` |  | A PowerShell function that initializes AWS state from a state file if enabled. |
| `Set-AWSProfile` | `asp` | A PowerShell function that sets the AWS profile for the current session with optional SSO operations. |
| `Set-AWSRegion` | `asr` | A PowerShell function that sets the AWS region for the current session. |
| `Switch-AWSProfile` | `acp` | A PowerShell function that switches AWS profiles with advanced support for MFA and role assumption. |
| `Update-AWSAccessKey` | `aws-change-key` | A PowerShell function that creates a new AWS access key pair and manages the transition from old keys. |
| `Update-AWSState` |  | A PowerShell function that updates the AWS state file with current profile and region information. |
