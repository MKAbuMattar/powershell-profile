...
<!-- Generated by Tools/Update-Readme.ps1 from the comment-based help in Conda.psm1. -->
<!-- Edit the help in the source, then re-run the tool. Changes here are overwritten. -->

| Command | Alias | Description |
| --- | --- | --- |
| `Get-CondaEnvs` |  | Gets list of Conda environments. |
| `Get-CondaInfo` |  | Gets Conda system information. |
| `Get-CondaVersion` |  | Gets the installed Conda version. |
| `Get-CurrentCondaEnv` |  | Gets the currently active Conda environment. |
| `Invoke-Conda` | `cn` | Base Conda command wrapper. |
| `Invoke-CondaActivate` | `cna` | Activate Conda environment. |
| `Invoke-CondaActivateBase` | `cnab` | Activate base Conda environment. |
| `Invoke-CondaClean` | `cncl` | Clean Conda caches and unused packages. |
| `Invoke-CondaCleanAll` | `cncla` | Clean all Conda caches and unused packages. |
| `Invoke-CondaConfig` | `cnconf` | Manage Conda configuration. |
| `Invoke-CondaConfigAdd` | `cnconfa` | Add Conda configuration value. |
| `Invoke-CondaConfigGet` | `cnconfg` | Get Conda configuration value. |
| `Invoke-CondaConfigRemove` | `cnconfr` | Remove Conda configuration value. |
| `Invoke-CondaConfigSet` | `cnconfs` | Set Conda configuration value. |
| `Invoke-CondaConfigShowSources` | `cncss` | Show Conda configuration sources. |
| `Invoke-CondaCreate` | `cnc` | Create Conda environment. |
| `Invoke-CondaCreateFromFile` | `cncf` | Create Conda environment from file. |
| `Invoke-CondaCreateName` | `cncr` | Create Conda environment with name. |
| `Invoke-CondaCreateNameYes` | `cncn` | Create Conda environment with name (auto-confirm). |
| `Invoke-CondaCreatePath` | `cncp` | Create Conda environment at path. |
| `Invoke-CondaDeactivate` | `cnde` | Deactivate current Conda environment. |
| `Invoke-CondaEnvExport` | `cnee` | Export Conda environment. |
| `Invoke-CondaEnvList` | `cnel` | List Conda environments. |
| `Invoke-CondaEnvUpdate` | `cneu` | Update Conda environment from file. |
| `Invoke-CondaInstall` | `cni` | Install packages with Conda. |
| `Invoke-CondaInstallYes` | `cniy` | Install packages with Conda (auto-confirm). |
| `Invoke-CondaList` | `cnl` | List installed packages. |
| `Invoke-CondaListExplicit` | `cnles` | Export explicit package list. |
| `Invoke-CondaListExport` | `cnle` | Export package list. |
| `Invoke-CondaRemove` | `cnr` | Remove packages with Conda. |
| `Invoke-CondaRemoveEnv` |  | Remove Conda environment. |
| `Invoke-CondaRemoveEnvName` | `cnrn` | Remove Conda environment by name. |
| `Invoke-CondaRemoveEnvPath` | `cnrp` | Remove Conda environment by path. |
| `Invoke-CondaRemoveYes` | `cnry` | Remove packages with Conda (auto-confirm). |
| `Invoke-CondaSearch` | `cnsr` | Search for packages. |
| `Invoke-CondaUpdate` | `cnu` | Update packages with Conda. |
| `Invoke-CondaUpdateAll` | `cnua` | Update all packages with Conda. |
| `Invoke-CondaUpdateConda` | `cnuc` | Update Conda itself. |
