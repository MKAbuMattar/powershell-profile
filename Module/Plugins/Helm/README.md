...
<!-- Generated by Tools/Update-Readme.ps1 from the comment-based help in Helm.psm1. -->
<!-- Edit the help in the source, then re-run the tool. Changes here are overwritten. -->

| Command | Alias | Description |
| --- | --- | --- |
| `Invoke-Helm` | `h` | A PowerShell function that wraps the Helm command. |
| `Invoke-HelmInstall` | `hin` | A PowerShell function that wraps `helm install`. |
| `Invoke-HelmSearch` | `hse` | A PowerShell function that wraps `helm search`. |
| `Invoke-HelmUninstall` | `hun` | A PowerShell function that wraps `helm uninstall`. |
| `Invoke-HelmUpgrade` | `hup` | A PowerShell function that wraps `helm upgrade`. |
