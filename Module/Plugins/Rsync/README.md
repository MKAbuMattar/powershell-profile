...
<!-- Generated by Tools/Update-Readme.ps1 from the comment-based help in Rsync.psm1. -->
<!-- Edit the help in the source, then re-run the tool. Changes here are overwritten. -->

| Command | Alias | Description |
| --- | --- | --- |
| `Get-RsyncVersion` |  | Get the version of installed rsync. |
| `Invoke-RsyncCopy` | `rsync-copy` | Copy files and directories using rsync with archive, verbose, compression, and progress. |
| `Invoke-RsyncMove` | `rsync-move` | Move files and directories using rsync (copy + remove source files). |
| `Invoke-RsyncUpdate` | `rsync-update` | Update files and directories using rsync (skip existing newer files). |
| `Sync-RsyncDirectories` | `rsync-synchronize` | Synchronize directories using rsync (two-way sync with deletion). |
| `Test-RsyncPath` |  | Test if a path is accessible for rsync operations. |
