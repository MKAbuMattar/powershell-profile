...
<!-- Generated by Tools/Update-Readme.ps1 from the comment-based help in Kubectl.psm1. -->
<!-- Edit the help in the source, then re-run the tool. Changes here are overwritten. -->

| Command | Alias | Description |
| --- | --- | --- |
| `Invoke-Kubectl` | `k` | A PowerShell function that wraps the kubectl command. |
| `Invoke-KubectlAllNamespaces` | `kca` | A PowerShell function that wraps kubectl commands with --all-namespaces flag. |
| `Invoke-KubectlApplyFile` | `kaf` | A PowerShell function that wraps `kubectl apply -f`. |
| `Invoke-KubectlConfigCurrentContext` | `kccc` | A PowerShell function that wraps `kubectl config current-context`. |
| `Invoke-KubectlConfigDeleteContext` | `kcdc` | A PowerShell function that wraps `kubectl config delete-context`. |
| `Invoke-KubectlConfigGetContexts` | `kcgc` | A PowerShell function that wraps `kubectl config get-contexts`. |
| `Invoke-KubectlConfigSetContext` | `kcsc` | A PowerShell function that wraps `kubectl config set-context`. |
| `Invoke-KubectlConfigUseContext` | `kcuc` | A PowerShell function that wraps `kubectl config use-context`. |
| `Invoke-KubectlDelete` | `kdel` | A PowerShell function that wraps `kubectl delete`. |
| `Invoke-KubectlDeleteFile` | `kdelf` | A PowerShell function that wraps `kubectl delete -f`. |
| `Invoke-KubectlDeletePods` | `kdelp` | A PowerShell function that wraps `kubectl delete pods`. |
| `Invoke-KubectlDescribePods` | `kdp` | A PowerShell function that wraps `kubectl describe pods`. |
| `Invoke-KubectlEditPods` | `kep` | A PowerShell function that wraps `kubectl edit pods`. |
| `Invoke-KubectlExecInteractive` | `keti` | A PowerShell function that wraps `kubectl exec -t -i`. |
| `Invoke-KubectlGetEvents` | `kge` | A PowerShell function that wraps `kubectl get events --sort-by=".lastTimestamp"`. |
| `Invoke-KubectlGetEventsWatch` | `kgew` | A PowerShell function that wraps `kubectl get events --sort-by=".lastTimestamp" --watch`. |
| `Invoke-KubectlGetPods` | `kgp` | A PowerShell function that wraps `kubectl get pods`. |
| `Invoke-KubectlGetPodsAll` | `kgpall` | A PowerShell function that wraps `kubectl get pods --all-namespaces -o wide`. |
| `Invoke-KubectlGetPodsAllNamespaces` | `kgpa` | A PowerShell function that wraps `kubectl get pods --all-namespaces`. |
| `Invoke-KubectlGetPodsLabels` | `kgpl` | A PowerShell function that wraps `kubectl get pods -l`. |
| `Invoke-KubectlGetPodsNamespace` | `kgpn` | A PowerShell function that wraps `kubectl get pods -n`. |
| `Invoke-KubectlGetPodsShowLabels` | `kgpsl` | A PowerShell function that wraps `kubectl get pods --show-labels`. |
| `Invoke-KubectlGetPodsWatch` | `kgpw` | A PowerShell function that wraps `kubectl get pods --watch`. |
| `Invoke-KubectlGetPodsWide` | `kgpwide` | A PowerShell function that wraps `kubectl get pods -o wide`. |
