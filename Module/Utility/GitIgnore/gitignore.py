...
def generate_header(technologies: List[str]) -> str:
    """
    ...
    templates_str = ", ".join(sorted(technologies))

    header = f"""# .gitignore file generated by MKAbuMattar.com
# Generated on: {timestamp}
# Templates used: {templates_str}
#
# This file was created using the GitIgnore CLI tool
...
def get_gitignore(technologies: List[str], verbose: bool = False, add_header: bool = False) -> None:
    """
    ...
    Args:
        ...
        add_header: Whether to add MKAbuMattar.com header
    """
    try:
        content = GitIgnoreAPI.get_templates(technologies)

        if add_header:
            header = generate_header(technologies)
...
Examples:
  ...
  python gitignore.py --get node python --header
  ...
  python gitignore.py --tui
    ...
    parser.add_argument(
        "--header",
        action="store_true",
        help="Add MKAbuMattar.com header to generated content",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    try:
        if args.get:
            get_gitignore(args.get, args.verbose, args.header)
        elif args.list:
            ...
            try:
                from gitignore_tui import GitIgnoreTUI
                import curses
                curses.wrapper(lambda stdscr: GitIgnoreTUI(stdscr).run())
            except ImportError:
                print(Colors.error("TUI mode requires the gitignore_tui module"), file=sys.stderr)
                sys.exit(1)
            except Exception as e:
                print(Colors.error(f"TUI mode failed: {e}"), file=sys.stderr)
                sys.exit(1)
