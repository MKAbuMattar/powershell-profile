...
function Format-InstallerScript {
    ...
    $lines.Add('')
    $lines.Add('    [Parameter()]')
    $lines.Add('    [switch]$Real,')
    $lines.Add('')
    $lines.Add('    [Parameter()]')
    $lines.Add('    [switch]$Sandbox')
    $lines.Add(')')
    $lines.Add('')
    ...
    $lines.Add('    )')
    $lines.Add('')
    $lines.Add('    # The first run on a machine goes into a throwaway directory, so a wrong click costs')
    $lines.Add('    # nothing while you are still finding out what the buttons do. Every run after that')
    $lines.Add('    # installs for real. -Real and -Sandbox override it.')
    $lines.Add('    $mode = Get-ProfileSetupMode -Real:$Real -Sandbox:$Sandbox')
    $lines.Add('    Write-ProfileSetupMode -Mode $mode')
    $lines.Add('')
    $lines.Add('    $target = @{}')
    $lines.Add('    if ($mode.InstallPath) { $target[''InstallPath''] = $mode.InstallPath }')
    $lines.Add('    if ($mode.ReceiptPath) { $target[''ReceiptPath''] = $mode.ReceiptPath }')
    $lines.Add('')
    $lines.Add('    $repository = Get-ProfileInstallerRepository -Branch $Branch')
    $lines.Add('')
    $lines.Add('    # Written before the picker opens, so closing it without installing anything still')
    $lines.Add('    # counts as having seen the list.')
    $lines.Add('    Set-ProfileSetupSeen -Repository $repository -Confirm:$false')
    $lines.Add('')
    $lines.Add('    if (-not $Console -and (Get-ProfileSetupWindowSupport).Supported) {')
    $lines.Add('        Show-ProfileSetupWindow -Repository $repository @target')
    $lines.Add('    }')
    $lines.Add('    else {')
    $lines.Add('        Show-ProfileSetup -Repository $repository @target')
    $lines.Add('    }')
    $lines.Add('}')
    $lines.Add('')
    $lines.Add('Invoke-ProfileInstaller -Console:$Console -Branch $Branch -Real:$Real -Sandbox:$Sandbox')
